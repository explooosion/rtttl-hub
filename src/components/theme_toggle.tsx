...
export function ThemeToggle({ mobile = false }: ThemeToggleProps) {
  ...
  return (
    <div
      className={clsx(
        "flex h-9 items-center gap-1 rounded-lg bg-gray-200 p-1 dark:bg-gray-800",
        mobile && "w-full",
      )}
    >
      {modes.map(({ value, icon: Icon }) => (
        <button
          ...
          className={clsx(
            "flex h-8 min-w-8 items-center justify-center gap-2 rounded-md px-1.5 text-sm transition-colors",
            mobile && "flex-1",
            currentMode === value
